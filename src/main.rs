...
fn main() {
    ...
    create_image(&device, &queue, [0.0, 0.0, 1.0, 1.0], "image.png");
}

fn test_buffer_copy(device: &Arc<vk::device::Device>, queue: &Arc<vk::device::Queue>) {
...
fn create_image(device: &Arc<vk::device::Device>, queue: &Arc<vk::device::Queue>, clear_color: [f32; 4], path: &str) {
